(function() {
  superselector = function()
	...
	{
		if($(oEvent.target).closest('#superselect').length == 0) 
		{
      oEvent.preventDefault(); //FIXME: this doesnt work,  but we need something like this in the future
  ...
  superselector.prototype.submitWatch = function  () {
    if (document.getElementById("superselect_generated").innerHTML.length) {
      alert('POST: Selector: ' + document.getElementById("superselect_generated").innerHTML);
    }
  };
...
//		return prefix + this.generateSelectorString() + suffix;
	...
	superselector.prototype.initialize = function(oSS)
	{
		...
		var superselectorHtml = 
            ...
            "<button id='watch' onclick=\"superselector.prototype.submitWatch()\" type='button'>Watch this element!</button>" +
					"</div>" +
				"</div>" +
